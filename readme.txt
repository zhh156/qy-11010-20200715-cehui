...
在做本项目中遇到了哪些难点？解决的思路是什么？
    ...
    讲一下项目的流程
    我的这个项目做的是关于测绘管理的，首先通过地址能够进入登录界面，登录界面有登录和注册的功能，登录进去之后有主页面、系统功能、测绘管理三大模块。
    在主页面中有关于测绘部门的地图、通过测绘部门的名称、注册时间进行动态的查询，同时也可以对测绘项目及测绘的成果通过名称、分类和创建时间进行动态查询
    在系统功能模块有对用户、角色、菜单、字典、部门的管理模块，在测绘管理模块进去首先看到的是对于部门信息的详细信息包括部门的法人代表、员工、特殊员工、
    以及部门中的一些资源信息，接着有单位审核、黑白名单、双随机抽查、项目管理、项目汇交、项目审核、测绘项目业务办理、数据统计、信息公开、公告栏


    与测绘部门相关的表总共9张：部门表、法人代表、员工、特殊员工、仪器设备、审核表、审核人员表、资源表、部门分值表
    与测绘项目或测绘成果相关的表总共5张：项目表、测绘成果提交表、审核表、资源表、审核人员表
    与用户相关的表共有4张：用户表、用户配置表（用来决定不同用户进入主界面展示的界面）、用户角色中间表、登录日志表
    与角色相关的表共有2张：角色表、角色菜单中间表
    与菜单相关的表共有1张：菜单表
    与部门相关的表共有1张：部门表
    与字典相关的表共有1张：字典表
    还有一张用于记录日志的表：日志表（进行的任何操作都进行记录）
    总共用到了22张表
